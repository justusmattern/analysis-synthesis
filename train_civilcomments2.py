...
def main():
    ...
    train_loader = prepare_dataset(
        args.data_dir, 'mr_neg', tokenizer,
    ...
    def test_plot(test_loader, num_iters):
        ...
        # test_iter = iter(test_loader); x_mask, x_tokens, y_mask, y_tokens, input_tokens, target_tokens, mask = next(test_iter)
        with tqdm(total=len(test_loader)) as pbar:
            for i, (x_mask, x_tokens, y_mask, y_tokens, input_tokens, target_tokens, mask, label, domains) in enumerate(
                    test_loader):
                y_mask = y_mask.to(device)
                y_tokens = y_tokens.to(device)
                x_mask = x_mask.to(device)
    ...
    while num_iters < args.iterations:
        ...
        torch.save(VAE.state_dict(), os.path.join(save_folder, f'model_approved_books_epoch{e}.pt'))
